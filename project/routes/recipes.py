...
def recipes():
    ...
    for result in rs:
        recipe = dict()
        recipe['id'] = result.id
        namesList = result.name.lower().replace('recipe', '').split('|')
        namesList = [name.strip().capitalize() for name in namesList]
        recipe['name'] = namesList
        ...
        recipe['ingredientsCount'] = len(ingredientsList)
